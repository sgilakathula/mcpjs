/**
 * __test__/mcpify.test.js
 *
 * Basic tests for mcpify using Node.js built-in test runner.
 * Run: node --test __test__/mcpify.test.js
 *
...
Module._resolveFilename = function(request, parent, isMain, options) {
  if (request === 'mcpify-darwin-x64' || request === 'mcpify-linux-x64-gnu' ||
      request === 'mcpify-win32-x64-msvc' || request.endsWith('.node')) {
    throw new Error('Mock: native not available')
  }
  return originalLoad.call(this, request, parent, isMain, options)
}

// Load mcpify with our mock binding injected
const mcpify = (() => {
  const m = new Module('mcpify-test', module)
  m.exports = {}
...
describe('McpServer', () => {
  it('creates a server with options', () => {
    const server = new mcpify.McpServer({ name: 'test', version: '1.0.0' })
    assert.ok(server)
  })
  ...
  it('registers a tool and returns this (chainable)', () => {
    ...
    const result = server.tool('my_tool', { description: 'A test tool' }, async () => ({
      content: [mcpify.textContent('hello')]
    }))
  ...
  it('registers a definePlugin() plugin', async () => {
    const myPlugin = mcpify.definePlugin('my-plugin', async (server, opts) => {
...
describe('Content helpers', () => {
  ...
  it('resourceContent creates correct structure', () => {
    const c = mcpify.resourceContent('file:///test.txt', 'text/plain')
...
describe('JSON-RPC utilities', () => {
  it('jsonrpcOk creates a valid response', () => {
    const raw = mcpify.jsonrpcOk('req-1', JSON.stringify({ status: 'ok' }))
  ...
  it('jsonrpcError creates a valid error response', () => {
    const raw = mcpify.jsonrpcError('req-2', -32601, 'Method not found')
    const parsed = JSON.parse(raw)
  ...
  it('parseJsonrpc extracts method and id', () => {
    const parsed = mcpify.parseJsonrpc(JSON.stringify({
      jsonrpc: '2.0',
      id: '42',
      method: 'tools/list',
      params: {}
...
describe('definePlugin', () => {
  it('returns a plugin object', () => {
    const plugin = mcpify.definePlugin('my-plugin', async () => {})
